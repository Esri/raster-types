        ...
        return [
            {
                ...
                # if the input raster is a raster proxy then
                # use'*.yaml;RasterProxy'
...
# return doc
    ...
    # to read the yaml file located on S3 over s3:// protocol
    def readYamlS3_boto3(self, bucket, path):
            ...
            if tag == 'be':
                ...
                if (_yamlpath.startswith("http")):
                    ...
                    if bluePath == greenPath:
                        ...
                        prjString = arcpy.SpatialReference(
                            spatialId).exportToString()
                        srsEPSG = spatialId
                    ...
                    # giving a start index of 5 will ensure that the / from
                    # s3:// is not returned.
                    ...
                    if bluePath == greenPath:
                        ...
                        # if the files on disk are raster proxies.
                        if itemURI['uriProperties']['rpflag'] == 1:
                ...
                # here noBands refers to the number of bands present in each
                # tiff being used
                if tag == 'fc':
                    nRows = 4000
                    nCols = 4000
                    ...
                    nCols = 4000
                    noBands = 1
                    # different bands have different data type, hence not
                    # defined
                ...
                if (_yamlpath.startswith("http")):
                    ...
                    if tag == 'fc':
                        FC01 = self.embedMRF_fc(
                            inputDir,
                            (doc['image']['bands']['BS']['path']),
                            maxX,
                            maxY,
                            minX,
                            minY,
                            ...
                            protocol,
                            cachePath)
                        FC02 = self.embedMRF_fc(
                            inputDir,
                            (doc['image']['bands']['NPV']['path']),
                            maxX,
                            maxY,
                            minX,
                            minY,
                            ...
                            protocol,
                            cachePath)
                        FC03 = self.embedMRF_fc(
                            inputDir,
                            (doc['image']['bands']['PV']['path']),
                            maxX,
                            maxY,
                            minX,
                            minY,
                            ...
                            protocol,
                            cachePath)
                        FC04 = self.embedMRF_fc(
                            inputDir,
                            (doc['image']['bands']['UE']['path']),
                            maxX,
                            maxY,
                            minX,
                            minY,
                        ...
                        MC01 = self.embedMRF(
                            inputDir,
                            (doc['image']['bands']['canopy_cover_class']['path']),
                            maxX,
                            maxY,
                            minX,
                            minY,
                        ...
                        WFS01 = self.embedMRF(
                            inputDir,
                            (doc['image']['bands']['confidence']['path']),
                            maxX,
                            maxY,
                            minX,
                            minY,
                        ...
                        WFS02 = self.embedMRF(
                            inputDir,
                            (doc['image']['bands']['wofs_filtered_summary']['path']),
                            maxX,
                            maxY,
                            minX,
                            minY,
                        ...
                        WSS01 = self.embedMRF(
                            inputDir,
                            (doc['image']['bands']['count_clear']['path']),
                            maxX,
                            maxY,
                            minX,
                            minY,
                        ...
                        WSS02 = self.embedMRF(
                            inputDir,
                            (doc['image']['bands']['count_wet']['path']),
                            maxX,
                            maxY,
                            minX,
                            minY,
                        ...
                        WSS03 = self.embedMRF(
                            inputDir,
                            (doc['image']['bands']['frequency']['path']),
                            maxX,
                            maxY,
                            minX,
                            minY,
                        ...
                        W01 = self.embedMRF(
                            ...
                            cachePath)
                        endIndex = (
                            doc['image']['bands']['water']['path']).rfind("_")
                        # the display name in the itemURI will be changed to
                        # this.
                        fileName = doc['image']['bands']['water']['path'][0:endIndex]
                    ...
                    if tag == 'fc':
                        FC01 = os.path.join(
                            yamldir, (doc['image']['bands']['BS']['path']))
                        nCols, nRows = self.returnVals(FC01)
                        FC02 = os.path.join(
                            yamldir, (doc['image']['bands']['NPV']['path']))
                        nCols, nRows = self.returnVals(FC02)
                        FC03 = os.path.join(
                            yamldir, (doc['image']['bands']['PV']['path']))
                        nCols, nRows = self.returnVals(FC03)
                        FC04 = os.path.join(
                            yamldir, (doc['image']['bands']['UE']['path']))
                        ...
                        MC01 = os.path.join(
                            yamldir, (doc['image']['bands']['canopy_cover_class']['path']))
                        nCols, nRows = self.returnVals(MC01)

                        MC02 = os.path.join(
                            yamldir, (doc['image']['bands']['extent']['path']))
                        ...
                        WFS01 = os.path.join(
                            yamldir, (doc['image']['bands']['confidence']['path']))
                        nCols, nRows = self.returnVals(WFS01)

                        WFS02 = os.path.join(
                            yamldir, (doc['image']['bands']['wofs_filtered_summary']['path']))
                        ...
                        WSS01 = os.path.join(
                            yamldir, (doc['image']['bands']['count_clear']['path']))
                        nCols, nRows = self.returnVals(WSS01)

                        WSS02 = os.path.join(
                            yamldir, (doc['image']['bands']['count_wet']['path']))
                        nCols, nRows = self.returnVals(WSS02)

                        WSS03 = os.path.join(
                            yamldir, (doc['image']['bands']['frequency']['path']))
                        ...
                        W01 = os.path.join(
                            yamldir, (doc['image']['bands']['water']['path']))
            ...
            if (tag == "be"):
                ...
                if beFlag == 0:
                    ...
                    try:
                        ...
                        arcpy.AddMessage(
                            "No valid geometry to update the footprint")
                ...
                with open(path, 'r') as csvfile:
                    ...
                    for row in reader:
                        ...
                        if (filename.startswith("http")
                                ):  # if the csv list contains a list of s3 urls
                            yield filename
